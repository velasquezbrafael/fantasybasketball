...
import {
  computeWeeklyWinnerRows,
  computeWeeklyWinnerTotals,
...
export default async function WeeklyWinnersPage() {
  ...
  const rows = computeWeeklyWinnerRows(
    matchups,
            ...
            <div key={row.matchupPeriodId} className="p-4 flex items-center gap-4 text-sm card-hover">
                ...
                <>
                  ...
                  <Link href={`/teams/${row.teamEspnIds[0]}`} className="flex items-center gap-2 flex-1 min-w-0 hover:text-accent transition-colors">
                    {row.teamEspnIds.slice(0, 1).map((id) => (
                      <TeamLogo key={id} logo={teamFor(id)?.logo} name={nameFor(id)} size={26} />
                    ))}
                  ...
                  <span className="tabular-nums font-semibold shrink-0 text-muted">{row.record}</span>
                </>
              )}
              ...
              {row.status === "decided" && (
                <>
                  ...
                  <span className="tabular-nums font-semibold shrink-0">{row.record}</span>
